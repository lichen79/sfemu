...
`crates/frontend/src/sf1panels.rs` for. Those are **Task 19's**, and they reach their
machine through two temporary helpers (Step 6) until then. Putting a rename and the
...
- Task 19 converts the loop's eight remaining `&Cps1` helpers, deletes `cps1`,
...
11. **`Sf1` has no `set_audio_stats`.** CPS-1's is on `SoundBoard` (`sound.rs:342`),
    ...
    below. Adding the door is Task 19's, with the loop.
...
fn draw_sf1(buf: &mut [u32], m: &Sf1) {
    ...
    // a board that has never been asked. Task 19 adds the door with the loop.
    line(
        buf,
        &mut row,
        &format!(
...
///
/// ⚠️ **Temporary, and Task 19 deletes it.** This task converts `run`'s signature so
/// the panels can dispatch on the board, but the loop's eight other helpers — and the
/// graphics viewer behind three of them — still take `&Cps1`. Converting those is
/// Task 19's, together with the SF1 pixel path; splitting it here would put a rename
/// and the graphics fork behind one review, which is the mistake Task 17 was split to
/// avoid.
///
/// The `unreachable!` is honest rather than defensive: `main.rs` constructs only a
/// `Machine::Cps1` until Task 19 adds the SF1 arm, and a silent `return` here would
/// be a window that opens on nothing.
fn cps1(m: &mut Machine) -> &mut Cps1 {
    match m {
        Machine::Cps1(c) => c,
        Machine::Sf1(_) => unreachable!("main builds only Cps1 until Task 19"),
    }
}

/// The same, shared. See [`cps1`]; Task 19 deletes both.
...
`cps1_ref(m)`.** It could take `&Machine` and call `cpu_view()`, and Task 19 deletes it
either way; leaving it alone keeps this task's `loop_.rs` diff to argument sites only.

`main.rs` wraps the machine it builds:

```rust
    // ⚠️ One arm until Task 19 adds the SF1 driver's, at which point the board comes
...
becomes a `LoopOpts` field "when the SF1 driver lands" — which is Task 19, where
`BOARD_SF1` and `Sf1State` exist. A field with one possible value, set from a `match`
with one arm, is scaffolding that reads as a finished decision.
...
16,768,000 ns by a longer route. Task 19 changes it in the commit that makes the other
board reachable.
...
on a board that has never been asked; Task 19 adds the door with the loop.
...
&Machine the panel needs is then rejected, E0502. Task 19 converts those
eight together with the SF1 pixel path, where one signature change makes all
of it consistent in one review.

const BOARD and FramePacer::cps1() also stay. Both become board-derived in
Task 19; on a loop that can only hold a CPS-1 they would be a field with one
